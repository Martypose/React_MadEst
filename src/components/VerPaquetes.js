...
function Paquetes() {
  ...
  const fetchPaquetes = async () => {
    axios
      ...
      .then((response) => {
        ...
        if (montadoRef.current) setPaquetes(paquetes);
  ...
  const fetchMedidas = async () => {
    axios
      .get(`${process.env.REACT_APP_URL_API}/medidas`, {
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
          accessToken: localStorage.getItem("accessToken"),
      ...
      .then((response) => {
        ...
        if (montadoRef.current) setMedidas(medidas);
      });
  };

  function selectorPaquetes(
    ...
    if (montadoRef.current) {
      ...
      if (calidad === "Ninguno") {
        ...
        let idMedidasCalidadSeleccionada = medidas
          .filter((medida) => medida.calidad === calidad)
        ...
        paquetesMostrarCalidades = paquetesMostrarEstados.filter((paquete) =>
          idMedidasCalidadSeleccionada.includes(paquete.medida)
        );
      }
  ...
  return (
    <div className="contenido">
      <h1>Paquetes</h1>
      <SelectMedidas
        medidaMostrar={medidaMostrar}
        estadoMostrar={estadoMostrar}
        calidadMostrar={calidadMostrar}
      />
      <div className="contenedor">
        <div className="fixed_header">
          <h2>
            Hay {paquetesMostrar.length} paquetes de {medidaElegida}
          </h2>
                ...
                if (paquete.cantidades !== "0" && paquete.cantidades !== null) {
                  return (
                    <tr key={paquete.ID}>
                      ...
                      <td key={dameCalidad(paquete, medidas)}>
                        {dameCalidad(paquete, medidas)}
                      </td>
