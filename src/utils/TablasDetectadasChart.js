...
function TablasDetectadasChart({ data = [], agrupamiento }) {
  ...
  const { labels, datasets, totalVolumes } = useMemo(() => {
    const labels = Array.from(new Set(normalized.map(r => formatLabel(r.fecha))));
    ...
    plugins: {
      tooltip: {
        callbacks: {
          ...
          label: (context) => `${context.dataset.label}: ${Number(context.raw ?? 0).toFixed(2)} m³`,
        },
      },
  ...
  const chartData = labels?.length ? { labels, datasets, totalVolumes } : null;
